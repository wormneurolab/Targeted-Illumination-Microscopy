...
ImageAngle = (atan2(ImageY,ImageX)/(2*pi))*360; % for directionality based double thresholding. May not necessary if high speed is needed.
...
DensityRatioThres = prctile(RatioImage(:), 100 - Density,'all');% find the threshold value according to the density set
...
for m = 1:SubMatrixX
    for n = 1:SubMatrixY
        ...
        if ImageOutLocalSum >= DensityPixelNumber*6 % if subimage pixels with assigned '1' has a density greater than 3 times density, this means it is not sparse but relatively good.
            if ImageOutLocalSum >= DensityPixelNumber*8 % if subimage pixels with assigned ones has a density greater than 5 times density, this means it is dense area
                ImageOutLocalSumCase = 1;
            end
            if ImageOutLocalSum >= DensityPixelNumber*6 && ImageOutLocalSum < DensityPixelNumber*8 % relative dense but not too dense area
            ...
            switch ImageOutLocalSumCase
                case 1
                    ImageOutmnSuroundDensityThres = prctile(RatioImageLocal,100 - Density*6,'all'); % to reduce very dense area density in segmentation
                    ImageOutLocal(RatioImageLocal<ImageOutmnSuroundDensityThres) = 0;
                case 2
                    ImageOutmnSuroundDensityThres = prctile(RatioImageLocal,100 - Density*5,'all'); % to reduce relative dense area density in segmentation
                    ImageOutLocal(RatioImageLocal<ImageOutmnSuroundDensityThres) = 0;
            end
...
% you should use this 'ImageRatioOut' as the output of this function and
% delete/comment all the rest part of this code.
...
FiberSegment = logical(ImageRatioOut); % The segmentation is a combination of Neuf, Ratio, Local density constrain, directionality
